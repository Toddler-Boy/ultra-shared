...
void GUI_XYPad::paint ( juce::Graphics& g )
{
	...
	if ( const auto lineW = UI::lineWidth ( UI::lines::xy_pad ); UI::lines::visible ( lineW ) )
...
void GUI_XYPad::updatePosition ( const juce::MouseEvent& evt )
{
	...
	const auto	travelX = std::max ( 1.0f, bounds.getWidth () );
	const auto	travelY = std::max ( 1.0f, bounds.getHeight () );

	const auto	x = int ( std::clamp ( ( evt.position.x - bounds.getX () ) / travelX, 0.0f, 1.0f ) * 200.0f - 100.0f );
	const auto	y = int ( std::clamp ( ( evt.position.y - bounds.getY () ) / travelY, 0.0f, 1.0f ) * 200.0f - 100.0f );
